...
def login():
    ...
    if form.validate_on_submit():
        ...
        if f.validate_user(form.user_email.data,form.user_pass.data) == 2:
            return dashboard(f.get_user_first_name(form.user_email.data), form.user_email.data, m.get_possible_matches(form.user_email.data), l.get_mutual_likes(form.user_email.data))
...
@app.route('/dashboard')
def dashboard(name = None, user_info = None, comp = None, mutual_likes = None):
    
    return render_template('dashboard.html',user_name = name, user=user_info, comp = comp, mutual_likes = mutual_likes)
